...
public class GameManager : MonoBehaviour, IWinObserver, ILoseObserver
{
    ...
    [SerializeField] RectTransform successImage, swordFailImage, youDiedImage, groundImage;
    float firstImageScale = 10f;
    ...
    IEnumerator Fail_Delay()
    {
        ...
        swordFailImage.localScale = new Vector3(firstImageScale, firstImageScale, firstImageScale);
        StartCoroutine(panelScaleSet(swordFailImage));
        yield return new WaitForSeconds(0.5f);

        youDiedImage.localScale = new Vector3(firstImageScale, firstImageScale, firstImageScale);
        StartCoroutine(panelScaleSet(youDiedImage));
        yield return new WaitForSeconds(0.5f);

        groundImage.localScale = new Vector3(1, 0, 1);
        StartCoroutine(panelScaleSet2(groundImage));
        yield return new WaitForSeconds(1f);
        groundImage.transform.GetChild(0).gameObject.SetActive(true);
        groundImage.transform.GetChild(1).gameObject.SetActive(true);
        scoreCounter();
   }
    public void scoreCounter()
    {
       
        LeanTween.value(0, Globals.currentBox, 0.2f).setOnUpdate((float val) =>
        {
            killedText.text = val.ToString("N0");
        });//.setOnComplete(() =>{});
        //PlayerPrefs.SetInt("money", Globals.moneyAmount);
    ...
    IEnumerator panelScaleSet2(RectTransform image)
    {
        float counter = 0;
        while (counter < 1f)
        {
